...
exports.whatsappMedicamento = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Mensaje de medicamento enviado correctamente.' });
  } catch (error) {
    console.error('Error al enviar mensaje de medicamento:', error.message);
    res.status(500).json({ error: 'Error al enviar mensaje de medicamento.' });
  }
};

exports.whatsappCitaMedica = async (req, res) => {
  try {
    const { numero } = req.body;
    ...
    res.status(200).json({ message: 'Mensaje de cita médica enviado correctamente.' });
  } catch (error) {
    console.error('Error al enviar mensaje de cita médica:', error.message);
    res.status(500).json({ error: 'Error al enviar mensaje de cita médica.' });
  }
};